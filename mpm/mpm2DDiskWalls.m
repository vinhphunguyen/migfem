...
Ew  = 1e9;            % Young's modulus of the wall
E   = 1;            % Young's modulus of the ball
nu  = 0.3;             % Poisson ratio
rho = 1000;            % density
rhoW= rho*1e9;            % density
kappa = 3-4*nu;        % Kolosov constant
mu    = E/2/(1+nu);    % shear modulus

v0   = 1e5;              % initial particle velocity

stressState ='PLANE_STRAIN'; % either 'PLANE_STRAIN' or "PLANE_STRESS
Cball       = elasticityMatrix(E,nu,stressState);
Cwall       = elasticityMatrix(Ew,nu,stressState);
%D           = inv(C);

vtkFileName  = 'mpm2DDiskWall';
vtkFileName1  = '../results/mpm2DDiskWallGrid';
...
numx2 = numx2 + 4;      % number of elements along X direction
numy2 = numy2 + 4;      % number of elements along Y direction
...
% mesh for the rigid walls

leftElems  = [2:numx2:numx2*(numy2-1)+2];
rightElems = [numx2-1:numx2:numx2*numy2-1];

rigidElems = [leftElems rightElems];
...
pCount = numelem*size(W,1)+4*length(rigidElems);
...
%% wall particles
[W,Q]=quadrature( 2, 'GAUSS', 2 ); % two point quadrature

for i = 1:length(rigidElems)
    e    = rigidElems(i);
    sctr = element(e,:);
    pts  = node(sctr,:);
    for q=1:size(W)
...
plot_mesh(node,element(leftElems,:),'Q4','b-',1.2);
plot_mesh(node,element(rightElems,:),'Q4','b-',1.2);
