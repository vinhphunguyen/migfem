...
%
% Thin cylinder shell. Full model is analyzed.
% Be careful with C^-1 points and C^0 points.
%
% Vinh Phu Nguyen,
% Cardiff University
% nvinhphu@gmail.com
...
% boundary nodes
nodesBottom  = find(abs(controlPts(:,3))<EPS)';
nodesTop     = find(abs(controlPts(:,3)-L)<EPS)';

% nodes with C^-1 continuity
% starting and ending control points

nodesStart1  = find(abs(controlPts(:,1)-R)<EPS)';
nodesStart2  = find(abs(controlPts(:,2)-0)<EPS)';
nodesC0      = intersect(nodesStart1,nodesStart2);

% nodes with C^0 continuity

nodes1       = nodesStart1;
nodes2       = find(abs(controlPts(:,1)+R)<EPS)';
nodes3       = find(abs(controlPts(:,2)-R)<EPS)';
nodes4       = find(abs(controlPts(:,2)+R)<EPS)';

% the constraint reads
%

xConsNodes = unique([nodesBottom nodesTop]);
yConsNodes = xConsNodes;
zConsNodes = xConsNodes;
...
plot3(controlPts(nodesC0,1),controlPts(nodesC0,2),controlPts(nodesC0,3),'ys');
...
for e=1:noElems
    ...
    sctrfx          = 3*sctr-2; % scatter for distributed force
    sctrfy          = 3*sctr-1; % scatter for distributed force
    ...
    for gp=1:size(W,1)
        ...
        f(sctrfx)  = f(sctrfx)  + Fx * R' * J1 * J2 * wt;
        f(sctrfy)  = f(sctrfy)  + Fy * R' * J1 * J2 * wt;
    end
end
...
for i=1:length(nodesC0)-1
    sctr  = [nodesC0(i) nodesC0(i+1)];
    sctrx = 3*sctr-2;
    sctry = 3*sctr-1;
    sctrz = 3*sctr-0;
...
outfileVTU  = '../results/thinCylinderShell.vtu';
