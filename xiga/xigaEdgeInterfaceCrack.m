...
%
% Interfacial Edge crack plate in tension (Matsumto, EFM 2000)
...
global p q controlPts weights element node xCrack xTips crack_node ...
       ep CHI chi weakEnrFunc jDomainFac elementV
...
inclusionEnrId  = 0; % =3: weak discontinuity enrichment
                     % =0: no enrichment
jDomainFac      = 4.; % J-integral domain = jDomainFac*area tip elem
...
%lineInterfaceCrackEdgeData
%lineInterfaceCrackEdgeC1Data
lineInterfaceCrackEdgeC2Data
%lineInterfaceCrackEdgeC0P3Data
%lineInterfaceCrackEdgeC0LinesP2Data

leftEdge     =  find(controlPts(:,1)==0); %left edge
rightEdge    =  find(controlPts(:,1)==L); %right edge
topNodes     =  find(controlPts(:,2)==3*L); %top edge
...
if (c0line==0)
    ...
    for i=1:noElemsU
        topEdgeMesh(i,:) = topNodes(2*i-1:2*i-1+p)';
        botEdgeMesh(i,:) = botNodes(2*i-1:2*i-1+p)';
    end
end
...
for e=1:noElems
    ...
    for gp=1:size(W,1)
        ...
        % Q4 shape functions for enrichment part
...
% L = ichol(K,struct('michol','on'));
% [U,fl1,rr1,it1,rv1] = pcg(K,f,1e-8,100,L,L');
...
% 
% figure
% plot_field(node,elementV,'Q4',epsYY);
% colorbar
...
l   = 2*a;
K1r = KI*cos(ep*log(l)) - KII*sin(ep*log(l));
K2r = KI*sin(ep*log(l)) + KII*cos(ep*log(l));

F1 = K1r/sigma0/sqrt(pi*a)
F2 = K2r/sigma0/sqrt(pi*a)
