...
c0points = [];
for i=2:length(uKnotVec)-1
    xii = uKnotVec(i);
    if multiplicity(i) == p 
        pnt = nrbeval(originalCurve,xii);
        c0points = [c0points;pnt'];
    end
end

% find indices of CPs with C^0 continuity

[~, index]  = ismember(c0points(:,1),controlPts(:,1));
fixedPntIds = [1 index' size(controlPts,1)];
...
noPts = [20 20 20 20 20 20 20]; % PROBLEM DEPENDENT!!!
...
for ik=1:noElems
    ...
    if (mult == p)
        ...
        pts  = controlPts(ielements.element(ik-1,:),:);
        x1   = N    * pts;
        dx1  = dNdxi* pts;
        dx1  = dx1/norm(dx1);
...
% initial CPs of the offset curve = CPs of the original curve
% except the first and last control points
% and points with C^0 continuity
