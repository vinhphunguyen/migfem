...
elementSet1 = [elemSet{1}{1} elemSet{2}{1} + data.mesh{1}.elemCount elemSet{3}{1} + data.mesh{1}.elemCount + data.mesh{2}.elemCount];
elementSet2 = [elemSet{1}{2} elemSet{2}{2} + data.mesh{1}.elemCount elemSet{3}{2} + data.mesh{1}.elemCount + data.mesh{2}.elemCount];
elementSet3 = [elemSet{1}{3} elemSet{2}{3} + data.mesh{1}.elemCount elemSet{3}{3} + data.mesh{1}.elemCount + data.mesh{2}.elemCount];
elementSet4 = [elemSet{1}{4} elemSet{2}{4} + data.mesh{1}.elemCount elemSet{3}{4} + data.mesh{1}.elemCount + data.mesh{2}.elemCount];
...
pause
%% write input file for jive

fileName = '~/code/jive/bezier/stiffener/stiffener.mesh';
writeMultiPatch3DToJive;

disp([num2str(toc),'  writing file for jive: done!!!'])
...
vtuFileName = '../results/stiffeners-curve1-jive';
resultFile = '~/code/jive/bezier/stiffener/stiffener.dat';
...
%    cc=V{it}.Section{1};
    ...
    U    = disp(:,2:end); % first column is node index
    for ip=1:patchCount
        vtuFile = strcat(vtuFileName,'-patch',num2str(ip),'-',num2str(it));
        %figure; hold on;
...
for ip = 1:patchCount
vtuFile = strcat(vtuFileName,'-patch',num2str(ip),'-',num2str(it),'.vtu');
