function ok=writeVTKForJive(mesh,vMesh,vtuFile,U,damage,materials,matMap)

 
% get information from mesh
noElems = mesh.noElems;
p       = mesh.p;
q       = mesh.q;
noPtsX  = mesh.noPtsX;
noPtsY  = mesh.noPtsY;
...
elRangeU=mesh.elRangeU;
elRangeV=mesh.elRangeV;
index   = mesh.index;
...
%             strain          = B*[U(sctr,1);U(sctr,2)];
              stress(e,gp,1)  = N*damage(sctr);
            disp(e,gp,1:3)  = N*[U(sctr,1) U(sctr,2) U(sctr,3)];
            %disp(e,gp,4)    = N*damage(sctr);
