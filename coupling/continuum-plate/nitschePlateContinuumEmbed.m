...
nnx=numx2+1;
nny=numy2+1;
node2t=square_node_array([0 0],[L 0],[L width],[0 width],nnx,nny);
inc_u=1;
inc_v=nnx;
...
%% define GPs on coupling interface----------------------------------------
...
for e=1:size(element2,1)  
    ...
    if elem2State(e) == 1
        [W,Q]=quadrature(  noGPs*5, 'GAUSS', 2 ); % 2 x2point quadrature
        x=zeros(size(W,1),1);
        for gp=1:size(W,1)
            pt      = Q(gp,:);                        
...
plot_mesh(node2+scaleFact*[Uz2 Uy2 Ux2],element2,elemTypeB,'b.-',1);
%colorbar
%axis off
...
stress=zeros(size(element1,1),size(element1,2),7);
...
for e=1:size(element1,1)
    ...
    for q=1:nn
        ...
        strain=B*U(sctrB);
        sigma=C*strain;
        stress(e,q,1:6)=sigma;
        stress(e,q,7)  = sqrt(sigma(1)^2+sigma(2)^2+sigma(3)^2-...
                    sigma(1)*sigma(2)-sigma(2)*sigma(3)-sigma(3)*sigma(1)+...
                    3*(sigma(4)^2+sigma(5)^2+sigma(6)^2));
    end
end   % of element loop

vtuFile1    = 'nitsche-plate-3D-continuumEmbed';
vtuFile2    = 'nitsche-plate-3D-plateEmbed';
vtuFileName = 'nitsche-plate-3DEmbed';
