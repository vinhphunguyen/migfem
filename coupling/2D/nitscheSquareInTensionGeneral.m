...
for e=1:numy1
    ...
    for q=1:size(W1)
        ...
        J0=dNdxi'*node1(sctrEdge,:); % also the tangent to Gamma_*^e
        detJ0=norm(J0);   
        J0 = J0/detJ0;
...
Ux2 = U(1+size(node1,1):numdofs);
Uy2 = U([1+size(node1,1):numdofs]+numdofs);
...
clf
hold on
plot_mesh(node1+scaleFact*[Ux1 Uy1],element1,elemType,'gs-',2.2);
plot_mesh(node2+scaleFact*[Ux2 Uy2],element2,elemType,'ro-',1.2);
colorbar
...
stress1=zeros(size(element1,1),size(element1,2),3);
stress2=zeros(size(element2,1),size(element2,2),3);
...
for e=1:size(element1,1)
    ...
    for q=1:nn
        ...
        strain=B*U(sctrB);
        stress1(e,q,:)=Ce*strain;
    end
end   % of element loop

for e=1:size(element2,1)
    sctrL=element2(e,:);
    sctr = sctrL + size(node1,1); 
    sctrB=[sctr sctr+numdofs];
    nn=length(sctr);
    Ce=C;
...
figure
clf
plot_field(node1+scaleFact*[Ux1 Uy1],element1,elemType,stress1(:,:,stressComp));
plot_field(node2+scaleFact*[Ux2 Uy2],element2,elemType,stress2(:,:,stressComp));
