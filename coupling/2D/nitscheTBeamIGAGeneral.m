...
K=sparse(2*numnodes,2*numnodes); % stiffness matrix
...
fid = fopen('tbeam-norm-p3.txt', 'at');
fprintf(fid, '%d %d %12.8e %12.8e\n', h,2*numnodes, disp_norm, energy_norm);
fclose(fid);

%% read the file and plot

% M1 = csvread('tbeam-norm-p1.txt');
% M2 = csvread('tbeam-norm-p2.txt');
% M3 = csvread('tbeam-norm-p3.txt');
% 
% figure
% set(gca,'FontSize',14)
% loglog(M1(:,1),M1(:,3),'k-','LineWidth',1.4);
% hold on
% loglog(M2(:,1),M2(:,3),'b-','LineWidth',1.4);
% xlabel('h')
% ylabel('e_{displacement}')
% legend('p=1 (slope=1.95)','p=2 (slope=2.97)')
% grid on
% 
% polyfit(log(M1(:,1)),log(M1(:,3)),1)
% polyfit(log(M2(:,1)),log(M2(:,3)),1)
% 
% 
% figure
% set(gca,'FontSize',14)
% loglog(M1(:,1),M1(:,4),'k-','LineWidth',1.4);
% hold on
% loglog(M2(:,1),M2(:,4),'b-','LineWidth',1.4);
% xlabel('h')
% ylabel('e_{energy}')
% grid on
% legend('p=1 (slope=0.98)','p=2 (slope=2.00)')
% 
% polyfit(log(M1(:,1)),log(M1(:,4)),1)
% polyfit(log(M2(:,1)),log(M2(:,4)),1)
% polyfit(log(M3(:,1)),log(M3(:,4)),1)
