...
vals=ones(length(couDom1),1);
nn=num*2; % this is problem dependent
for i=1:numyL
    vals((i-1)*nn+1:(i-1)*nn+nn) = i;
end
...
fine2VirtualMap = containers.Map(couDom1,vals);
...
for i=1:length(couDom1)                         
    ...
    cId = fine2VirtualMap(e);
    sctrL  = lagDom(cId,:); 
    sctrLL = element2(couDom2(cId),:); 
    sctrLB = [ sctrL sctrL+numdofs ]; 
...
hold on
plot_mesh(node1+scaleFact*[Ux1 Uy1],element1,elemType,'g.-',1);
