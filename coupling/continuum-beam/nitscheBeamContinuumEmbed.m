...
%% find void elements, cut elements
% void elements= beam elements overlapped by solid elements, hence
% do not contribute to the stiffness matrix.
% cut element (only one): special integration as in XFEM
...
for i=1:size(element2,1)
    if xx(i) < 0 && xx(i+1) < 0
        elem2State(i) = 0;           % void elements
    elseif xx(i) * xx(i+1) < 0
        elem2State(i) = 1;           % cut elements
    else
        elem2State(i) = 2;           % cut elements
    end
end

cutElem = find(elem2State==1);
...
inactiveNodes2 = element2(cutElem,:);

inactiveBeamNodes = setdiff(inactiveNodes1,inactiveNodes2)';

inactiveBeamNodes = inactiveBeamNodes + size(node1,1);
...
for e=1:numy1
    ...
    for q=1:size(W1)
        ...
        X2  = (2*x(1)-pts2(1)-pts2(2))/(pts2(2)-pts2(1));
...
uFixed=zeros(1,length(fixedNode)+length(inactiveBeamNodes))';  % a vector of u_x for the nodes
vFixed=zeros(1,length(fixedNode)+length(inactiveBeamNodes))';
...
for e=1:size(element2,1)
    ...
    pts   = node2(conn,:);
    [W,Q] = embedQuadrature1D(  noGPs, elem2State, L1, pts,e);
...
plot_mesh(node2+scaleFact*[zeros(numnode2,1) Ux2],element2,elemTypeB,'r.-',1);
...
xlabel('x')
ylabel('stresses at y=5.4')
grid on
%axis([0 5 -0.55 0])

%%

elems = numx1*(8)+1:numx1*9;
...
figure
clf
plot_field(node1+scaleFact*[Ux1 Uy1],element1,elemType,sigmaXX);
